...
export default function SubscriptionReactivationPage() {
    ...
    const fetchData = async () => {
      try {
        const [contactsRes, templatesRes] = await Promise.all([
          apiClient.get('/subscription-reactivation/dormant', {
  ...
  const handleBulkReactivate = async () => {
    ...
    try {
      ...
      if (scheduledDate && scheduledTime) {
        ...
        const response = await apiClient.post('/subscription-reactivation/reactivate', {
        ...
        setTimeout(() => {
          const fetchDormantContacts = async () => {
            try {
              const response = await apiClient.get('/subscription-reactivation/dormant', {
                params: { days: daysInactive, limit: 50 },
              });
  ...
  // Get min datetime for scheduling (now + 1 minute)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold tracking-tight text-gray-900">Subscription Reactivation</h1>
        <p className="text-gray-600">
          AI-powered subscription reactivation and engagement
        </p>
      </div>
